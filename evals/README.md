...
The current `0.5.0` package is therefore still a preview. The repository preserves one synthetic baseline-versus-Skill comparative adjudication, but real multi-hour Codex and Claude project trials are still required before stronger claims are made. Future trials must report model and host versions, scenario artifacts, compliance, rationalizations, overhead, failures, and limitations.
