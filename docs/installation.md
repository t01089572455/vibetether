...
This command follows the current `main`. The `?v=0.6.3` suffix is a
...
not activate repository documents. Use the manifest-declared `truth_index`
(normally `.vibetether/TRUTH.md`) directly or ask the Agent to search for
candidates and confirm them one at a time.
...
```sh
npx --yes --package=https://codeload.github.com/t01089572455/vibetether/tar.gz/refs/heads/main?v=0.6.3 vibetether init --project . --agent both --profile core --no-auto-bundles --yes
npx --yes --package=https://codeload.github.com/t01089572455/vibetether/tar.gz/refs/heads/main?v=0.6.3 vibetether init --project . --agent both --profile extended --bundle web --bundle production --yes
```
...
```sh
npx --yes --package=https://codeload.github.com/t01089572455/vibetether/tar.gz/refs/heads/main?v=0.6.3 vibetether init --project . --agent both --profile standard --dry-run
...
directory; and create the `.vibetether/` intent, blank user-owned `TRUTH.md`,
capability board, provider lock, checkpoint, experience index, and project-local
CLI launcher.
...
During an upgrade, an older project may already use `.vibetether/TRUTH.md` as a
prose authority document. VibeTether leaves it byte-for-byte unchanged, creates
the canonical index at `.vibetether/TRUTH-MAP.md`, and updates only the manifest
route. An occupied or malformed sidecar stops the update before project writes.
...
```sh
npx --yes --package=https://codeload.github.com/t01089572455/vibetether/tar.gz/refs/tags/v0.6.3 vibetether uninstall --project . --dry-run
npx --yes --package=https://codeload.github.com/t01089572455/vibetether/tar.gz/refs/tags/v0.6.3 vibetether uninstall --project . --yes
```
